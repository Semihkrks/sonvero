...
async function loadDrafts(page, type) {
  ...
  try {
    ...
    tbody.innerHTML = drafts.map((d, index) => {
      ...
      const typeStr = info.InvoiceType || 'Satış';
      const amount = info.PayableAmount || 0;
      ...
      const alias = customer.ReceiverAlias || customer.Alias || 'Belirtilmemiş';
      const profile = info.InvoiceProfile || 'Ticari Fatura';
