...
export async function renderOutgoingInvoices() {
...
async function loadOutgoing(page) {
  ...
  try {
    ...
    if (allInvoices.length === 0) {
      ...
      if (efOk || eaOk) {
        ...
        tbody.querySelector('#retryBtn')?.addEventListener('click', () => loadOutgoing(page));
      }
      updateFooter(page, 0);
      return;
    }
    ...
    tbody.querySelector('#retryBtn2')?.addEventListener('click', () => loadOutgoing(page));
    updateFooter(page, 0);
  }
}
...
function renderTable(page, invoices) {
  ...
  tbody.innerHTML = invoices.map((inv, idx) => {
    ...
    return `<tr>
      ...
      <td>${statusDisplay(inv)}</td>
      <td>${answerDisplay(inv)}</td>
...
async function handleAction(act, uuid, source) {
  switch (act) {
    case 'gib':
      try {
        const res = await EInvoice.getSaleStatus(uuid);
        showModal({
...
async function downloadFile(uuid, source, type) {
  try {
    let res;
    if (source === 'earsiv') {
      res = type === 'pdf' ? await EArchive.getInvoicePdf(uuid) : await EInvoice.getSaleHtml(uuid); // EArchive has getInvoicePdf, missing XML handling directly, fallback handled if needed
